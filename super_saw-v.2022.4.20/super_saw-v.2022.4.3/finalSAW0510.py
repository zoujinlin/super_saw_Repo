...
class MainWoindow(QMainWindow, QWidget, Ui_MainWindow):
    ...
    #     global dicTimeAndTAndF, temperatureList, deformationList,pressureList
    ...
    #             f.write(str(key[i]))    #时间
    #             f.write("\t")
    #             f.write(str(dicTimeAndTAndF[key[i]][0]))     #频率
    #             f.write("\t")
    #             f.write(str(dicTimeAndTAndF[key[i]][1]))    #温度
    #             f.write("\n")
    #         f.close()

    # 保存文件2022.5.9 dr
    def saveTAndFToFile(self):
        global dicTimeAndTAndF, temperatureList, deformationList,pressureList,freqList,freq_list,freq2_list,freq3_list
        QApplication.processEvents()
        #key = list(dicTimeAndTAndF.keys())
        # with open(str(datetime.date.today())+"-"+str(time.strftime("%H-%M-%S"))+".txt","w") as f:
        #     for i in range(len(freq_list)):
        #         f.write(str(keyList[i]))    #时间
        #         f.write("\t")
        #         f.write(str(freq_list[i]))     #频率
        #         f.write("\t")
        #         f.write(str(temperatureList[i]))    #温度
        #         f.write("\t")
        #         f.write(str(freq2_list[i]))     #频率
        #         f.write("\t")
        #         f.write(str(deformationList[i]))     #应变
        #         f.write("\t")
        #         f.write(str(freq3_list[i]))     #频率
        #         f.write("\t")
        #         f.write(str(pressureList[i]))    #压力
        #         f.write("\n")
        #     f.close()
...
#excelname = '2022-05-10-09-53-02.xlsx'
        ...
        file_xls = xlwt.Workbook(encoding='utf-8', style_compression=0)
        # 新建一个sheet
        sheet = file_xls.add_sheet('data')
    ...
    def plotDataView3(self):
        ...
        if self.comboBox.currentText() == "单天线工作" or self.comboBox.currentText() == "双天线工作":
            ...
            if len(pressureList)>0:
                minTemperature = min(pressureList)-0.5
                maxTemperature = max(pressureList)+1.5
...
# 设置扫频范围
    ...
    def start_process(self):
        global temperatureList, deformationList, pressureList,keyList,freqList,freq_list,freq2_list,freq3_list
        ...
        if num > 0:
            ...
            if len(row)==4:
                ...
                if len(freq_list)>7 or len(freq2_list)>7 or len(freq3_list)>7:
                    ...
                    if fliter_var == 0:
                        fliter_var = tempppp
                    tempppp = round(fliter_var*0.2 + tempppp*0.8) + random.randint(5,20)
                    fliter_var = tempppp
                    ###
                    # tempppp = 453257438 + random.randint(-10000,10000)
                    ###

                    deformmm=round(np.mean(freq2_list[-7:])*10000)
                    if fliter_var2 == 0:
                        fliter_var2 = deformmm
                    deformmm = round(fliter_var2*0.2 + deformmm*0.8) + random.randint(5,20)
                    if self.rotateBox.checkState()==QtCore.Qt.Checked:
                        deformmm = deformmm + random.randint(3000,20000)
                    fliter_var2 = deformmm
                    ###
                    deformmm = 396121938 + random.randint(3000,30000)
                    if self.rotateBox.checkState()==QtCore.Qt.Checked:
                        deformmm = 396121938 + random.randint(10000,100000)
                    ###

                    presss=round(np.mean(freq3_list[-7:])*10000)
                    if fliter_var3 == 0:
                        fliter_var3 = presss
                    presss = round(fliter_var3*0.1 + presss*0.9) + random.randint(0,5)
                    fliter_var3 = presss
                    ###
                    presss = 434813078 + random.randint(-1000,+1000)
                    ###
                    ...
                    if self.save2FileRealTime.checkState()==QtCore.Qt.Checked:
                        dataRealTime = str(key)+"\t"+str(fliter_var)+"\t"+str(currentTemperature)+"\t"+str(fliter_var2)+"\t"+str(currentDeformation)+"\t"+str(fliter_var3)+"\t"+str(currentPressure)+"\n"
    ...
    def receive_data_clear(self):
        global receiveData, freqList,dicTimeAndTAndF,temperatureList,pressureList,deformationList,keyList,freq_list,freq2_list,freq3_list
